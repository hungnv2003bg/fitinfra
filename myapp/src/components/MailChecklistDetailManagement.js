...
export default function MailChecklistDetailManagement() {
  ...
  const labels = {
    vi: {
      title: "Quản lý danh sách mail nhận thông báo hoàn thành checklist",
      add: "Thêm mới",
      edit: "Chỉnh sửa",
      delete: "Xóa",
      email: "Email",
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/mail-recipients-checklist-detail/${id}`);
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingRecipient) {
        ...
        const checklistDoneType = typeRes.data;
        ...
        const payload = {
          ...
          enabled: values.enabled,
          note: values.note,
          typeMailRecipient: checklistDoneType
        };
        await axios.post('/api/mail-recipients-checklist-detail', payload);
      }
      notification.success({
        message: 'Hệ thống',
        description: t.saved,
