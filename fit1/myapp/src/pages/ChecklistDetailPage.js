...
import { Table, Button, Tag, Spin, message, Upload, Input, DatePicker, Select, Space, Modal, Descriptions, Form, List, notification } from "antd";
import { useParams, useNavigate } from "react-router-dom";
import { UploadOutlined, ArrowLeftOutlined, EyeOutlined, SwapOutlined, DeleteOutlined, MailOutlined } from "@ant-design/icons";
  ...
  const handleSendMail = async (record) => {
    try {
      ...
      notification.success({
        ...
        description: 'Đã gửi mail tới người thực hiện',
        placement: 'bottomRight',
      });
    } catch (error) {
      console.error('Error sending mail:', error);
      message.error({
        content: 'Gửi mail thất bại: ' + (error.response?.data?.message || error.message),
        placement: 'bottomRight'
      });
    } finally {
        ...
        return (
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
            ...
            <Button 
              ...
              title="Gửi mail nhắc việc cần hoàn thành"
              loading={sendingMail}
